...
async function seed() {
    if (process.env.NODE_ENV === "production") {
        throw new Error("Refusing destructive seed in production");
    }
    if (process.env.SEED_RESET !== "true") {
        throw new Error("Set SEED_RESET=true to allow destructive seeding");
    }
    await connectToDB();
    ...
    songDocs.forEach((song) => {
        song.artist = albums.find((album) =>
            album._id.equals(song.album),
        ).artist;
    });
    const songs = await Song.insertMany(songDocs);

    console.log("Seeding 502 users (2 demo users plus 500 regular users)...");
